    ...
    for epoch in range(100):
        ...
        for x, y in train:
            ...
            train.set_description(
                f"epoch: {epoch+1}/{300} Step: {count+1} dice_loss : {running_loss/count:.4f} iou_score: {1-running_loss/count:.4f}")
        ...
        with torch.no_grad():
            ...
            for x, y in val:
                ...
                acc_loss += acc
                val.set_description(
                    f"val_epoch: {epoch+1}/{300} Step: {count+1} dice_loss : {val_running_loss/count:.4f} iou_score: {1-val_running_loss/count:.4f}")
